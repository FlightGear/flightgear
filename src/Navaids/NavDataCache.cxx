...
#define AND_TYPED "AND type>=?2 AND type <=?3"
  ...
  // define a new octree node (with no children)
    ...
    isPosInAirway = prepare("SELECT rowid FROM airway_edge WHERE network=?1 AND a=?1");
    
    airwayEdgesFrom = prepare("SELECT airway, b FROM airway_edge WHERE network=?1 AND a=?1");
  }
...
bool NavDataCache::isInAirwayNetwork(int network, PositionedID pos)
{
  ...
  return d->execSelect(d->isPosInAirway);
}
