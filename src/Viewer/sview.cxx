...
{
    #if 0
    // Pseudo-code for creating from a property node, e.g. if we were to use
    // Svew for main view window.
    //
    SviewSteps(SGPropertyNode* node)
    {
        bool finished = false;
...
// Provided for callers of SviewCreate() where a osg::GraphicsContext was
// passed. This avoids the need to expose the SviewView class in sview.hxx.
//
...
struct SviewDouble : SviewView
{
    ...
    SviewDouble(
            osgViewer::View* view,
            const SviewSteps& local,
            const SviewSteps& remote
            )
    :
    SviewView(view),
    m_local(local),
    ...
    virtual bool update(double dt) override
    {
        ...
        // when a real solution is not possible.
...
struct SviewViewClone : SviewView
{
    ...
    SviewView(view)
    {
        ...
        if (view_config->getBoolValue("eye-fixed")) {
            ...
            // Add a step to set pitch and roll to zero, otherwise view from tower (as
            ...
            if (type == "lookat") {
                ...
                // config/target-z-offset-m seems to be +ve when moving
                // backwards relative to the aircraft, so we need to negate the
                // value we pass to SviewStepMove().
...
void SviewUpdate(double dt)
{
    ...
    for (size_t i=0; i<s_views.size(); /* inc in loop*/) {
        ...
        if (valid) {
            ...
            //
            // This doesn't work reliably with OpenSceneGraph-3.4. We seem
            // to sometimes get valid=false when a different window from
            // s_views[i] is closed. And sometimes OSG seems to end up sending
            // a close-window even to the main window when the user has closed
            // an sview window.
            //
        ...
        )
{
    ...
    SG_LOG(SG_GENERAL, SG_ALERT, "Creating new view");
